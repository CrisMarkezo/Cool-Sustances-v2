...
this.lifeBar.setScale(this.currentUiScale);
    ...
    preUpdate(t, dt) {
        ...
        if (this.cursors.left.isDown || this.wasd.left.isDown) { vx = -1; this.setFlipX(true); }
        else if (this.cursors.right.isDown || this.wasd.right.isDown) { vx = 1; this.setFlipX(false); }

        if (this.cursors.up.isDown || this.wasd.up.isDown) vy = -1;
        else if (this.cursors.down.isDown || this.wasd.down.isDown) vy = 1;

        if (vx !== 0 || vy !== 0) { this.lastMoveX = vx; this.lastMoveY = vy; }

        if (!this.isGrabbing && !this.isKnocked) {
            if (vx !== 0 || vy !== 0) {
                const velocity = new Phaser.Math.Vector2(vx, vy).normalize().scale(this.speed);
                this.body.setVelocity(velocity.x, velocity.y);
                if (!this.isAttacking && this.anims.currentAnim?.key !== 'cat_run') this.anims.play('cat_run');
            } else {
                this.body.setVelocity(0);
                if (!this.isAttacking && this.anims.currentAnim?.key !== 'cat_idle') this.anims.play('cat_idle');
            }
        }
    ...
    takeDamage(source) {
        ...
        this.health -= 10;
        if (this.health <= 0) { this.health = 0; if (this.body) this.body.setVelocity(0, 0); }
        ...
        if (source && this.health > 0) {
            const kb = new Phaser.Math.Vector2(this.x - source.x, this.y - source.y).normalize();
            this.body.setVelocity(kb.x * this.damageKnockbackForce, kb.y * this.damageKnockbackForce);
        }

        this.scene.time.delayedCall(250, () => { this.isKnocked = false; });
